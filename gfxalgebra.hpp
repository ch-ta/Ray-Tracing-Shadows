...
public:
  ...
  constexpr same_type operator-() const noexcept {
    same_type result;
    for (size_t i = 0; i < DIMENSION; ++i)
      result.elements_[i] = 0 - elements_[i];
    return result;
  }
  ...
  constexpr bool approx_equal(const same_type& other, scalar_type delta) const noexcept {
    for (size_t i = 0; i < DIMENSION; ++i) {
      if(!gfx::approx_equal(elements_[i], other.elements_[i], delta))
        return false;
    }
    return true;
  }
  ...
  grow(scalar_type default_value = 0) const noexcept {
    ...
    vector<scalar_type, NEW_DIMENSION> result(default_value);
    for (size_t i = 0; i < DIMENSION; i++)
  ...
  subvector(size_t start = 0) const noexcept {
    ...
    vector<scalar_type, NEW_DIMENSION> result;
    for (size_t i = 0; i < NEW_DIMENSION; i++, start++)
      result[i] = elements_[start];
    return result;
  }
  ...
  constexpr same_type cross(const same_type& rhs) const noexcept {
    ...
    same_type result;
    for (int i = 0; i < 3; i++)
      result[i] = elements_[(i + 1) % 3] * rhs.elements_[(i + 2) % 3] - elements_[(i + 2) % 3] * rhs.elements_[(i + 1) % 3];
    return result;
  }
...
public:
  ...
  constexpr same_type operator+ (const same_type& rhs) const noexcept {
    ...
    for (size_t r = 0; r < HEIGHT; r++) {
      for (size_t c = 0; c < WIDTH; c++)
        result.rows_[r][c] = rows_[r][c] + rhs.rows_[r][c];
    }
    return result;
  }
  ...
  constexpr same_type operator* (const scalar_type rhs) const noexcept {
    ...
    for (size_t r = 0; r < HEIGHT; r++) {
      for (size_t c = 0; c < WIDTH; c++)
        result.rows_[r][c] = rows_[r][c] * rhs;
    }
    return result;
  }
  ...
  operator* (const matrix<scalar_type, WIDTH, RESULT_WIDTH>& rhs) const noexcept {
    ...
    for (size_t r1 = 0; r1 < HEIGHT; r1++) {
      for (size_t c2 = 0; c2 < RESULT_WIDTH; c2++) {
        for (size_t c1 = 0; c1 < WIDTH; c1++)
          result[r1][c2] += rows_[r1][c1] * rhs[c1][c2];
      }
    }
    return result;
  }
  ...
  constexpr bool approx_equal(const same_type& other, scalar_type delta) const noexcept {
    for (size_t r = 0; r < HEIGHT; r++) {
      for (size_t c = 0; c < WIDTH; c++) {
        if (!gfx::approx_equal(rows_[r][c], other.rows_[r][c], delta))
          return false;
      }
    }
    return true;
  ...
  submatrix(size_t top_row = 0, size_t left_column = 0) const noexcept {
    ...
    for (size_t r = 0; r < NEW_HEIGHT; r++, top_row++) {
      size_t lc = left_column;  // reset to left_column
      for (size_t c = 0; c < NEW_WIDTH; c++, lc++)
        result[r][c] = rows_[top_row][lc];
    }
    return result;
  }
  ...
  scalar_type determinant() const noexcept {
    ...
    scalar_type result = 0;
    if (WIDTH == 2)
      result = rows_[0][0] * rows_[1][1] - rows_[1][0] * rows_[0][1];
    else if (WIDTH == 3) {
      result = 
          rows_[0][0] * (rows_[1][1] * rows_[2][2] - rows_[2][1] * rows_[1][2])
        - rows_[0][1] * (rows_[1][0] * rows_[2][2] - rows_[2][0] * rows_[1][2])
        + rows_[0][2] * (rows_[1][0] * rows_[2][1] - rows_[2][0] * rows_[1][1]);
    }
    return result;
  }
  ...
  vector<scalar_type, HEIGHT> solve(const vector<scalar_type, HEIGHT>& b) const noexcept {
    ...
    if (WIDTH == 2) {
      ...
      result[0] = X.determinant() / determinant();
      result[1] = Y.determinant() / determinant();
    }
    else if (WIDTH == 3) {
      same_type X;
      same_type Y;
      ...
      result[0] = X.determinant() / determinant();
      result[1] = Y.determinant() / determinant();
      result[2] = Z.determinant() / determinant();
    }
    return result;
  }
  ...
  static constexpr same_type identity() noexcept {
    static_assert(is_square(),
                  "only square matrices can be used to create identities");
    same_type result;
    for (size_t i = 0; i < HEIGHT; i++)
      result[i][i] = 1;
    return result;
...
// Convert vector to a one column matrix.
template <typename scalar_type,
          size_t DIMENSION>
...
vector<scalar_type, DIMENSION>::to_column_matrix() const noexcept {
  ...
  for (size_t i = 0; i < DIMENSION; i++)
    result[i][0] = elements_[i];
  return result;
}

// Convert vector to a one row matrix.
template <typename scalar_type,
          size_t DIMENSION>
